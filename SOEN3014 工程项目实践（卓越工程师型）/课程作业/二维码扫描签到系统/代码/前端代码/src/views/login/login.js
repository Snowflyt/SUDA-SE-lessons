...
export default {
  ...
  data () {
    const validateUsername = (rule, value, callback) => {
      if (!isvalidUsername(value)) {
        callback(this.$t('login.errorAccount'))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      if (value.length < 2) {
        callback(this.$t('login.errorPassword'))
      } else {
        callback()
      }
    }
  ...
  methods: {
    ...
    getQrcode () {
      ...
      const url =
        getApiUrl() + '/account/qrcode/generate?uuid=' + this.qrcode.uuid
    ...
    getQrcodeResult () {
      if (this.qrcode.activeName === 'second') {
        ...
        this.sleep(1000).then(() => {
          getQrcodeStatus({ uuid: me.qrcode.uuid })
            .then(res => {
              ...
              if (res.data.status === 'success') {
                ...
                this.loading = true
                this.$store
                  .dispatch('user/autoLogin', { token: res.data.token })
                  .then(() => {
                    this.loading = false
              ...
              if (
                me.qrcode.activeName === 'second' &&
                res.data.status === 'undo'
              ) {
                me.getQrcodeResult()
              }
            })
