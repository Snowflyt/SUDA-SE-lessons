  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('config.value')">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="scope.row.cfgValue.length > 50"
            title="点击查看全部参数值"
            @click="showCfgValDialog(scope.row)"
            >{{ scope.row.cfgValue.substr(0, 25) }}</el-button
          >
    ...
    <el-dialog :title="dialog.title" :visible.sync="dialog.show" width="50%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click.native="dialog.show = false">{{
          $t('button.close')
        }}</el-button>
      </span>
    </el-dialog>
